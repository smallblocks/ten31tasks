...
const shiftWorkday = (iso, n) => {
  let result = iso;
  const step = n > 0 ? 1 : -1;
  let remaining = Math.abs(n);
  while (remaining > 0) {
    result = shiftDay(result, step);
    if (!isWeekend(result)) remaining--;
  }
  return result;
};

const previousWorkday = (iso) => {
  let result = iso;
  while (isWeekend(result)) result = shiftDay(result, -1);
  return result;
};

const todayWorkday = () => previousWorkday(today());
...
function useSettings() {
  const [settings, setSettings] = useState({ skipWeekends: true });
  useEffect(() => {
    api("/settings")
      .then((s) => setSettings({ skipWeekends: !!s.skip_weekends }))
      .catch(() => {});
  }, []);
  return settings;
}
...
const HammerIcon = () => <svg width="18" height="18" viewBox="0 0 18 18" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><g transform="rotate(30 9 9)"><rect x="4" y="2" width="10" height="5" rx="1"/><line x1="9" y1="7" x2="9" y2="16" strokeWidth="3"/></g></svg>;
